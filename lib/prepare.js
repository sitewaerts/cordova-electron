...
/**
 * @typedef {Object.<string, string>} ResourceMapEntry
 */

/**
 * @typedef {Array<ResourceMapEntry | null | undefined>} ResourceMap
 */
 ...
 * @param {string} path
 * @param {Array<OptionalIcon>} sources
 * @void
 */
function _applyIcon (resourceMap, cordovaProject, path, sources) {
    for(const icon of sources){
        if(icon){
            const mapped = mapResources(cordovaProject.root, icon.src, path + icon.extension);
            if(mapped){
                resourceMap.push(mapped);
                return;
            }
            else {
                events.emit('warn', `Cannot resolve icon ${icon.src}.`);
            }
        }
    }
}
...
function applyIcon (resourceMap, cordovaProject, path, ...sources) {
    _applyIcon(resourceMap, cordovaProject, path, sources)
}
...
function applyTypedIcon(resourceMap, cordovaProject, path, extension, ...sources) {
    _applyIcon(resourceMap, cordovaProject, path, sources.filter((icon)=>{return icon?.extension === extension}));
}
        ...
        if(element) {
            ...
            if (elementKeys?.length) {
                elementKeys.forEach((key)=>{
                    ...
                    if(value)
                        fs.cpSync(path.join(rootDir, key), value, {recursive: true});
                })
            }
        }
    });
