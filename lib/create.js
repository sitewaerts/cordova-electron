...
*/
    ...
    if(!fs.existsSync(path.join(rootDir, 'bin/templates/platform_www/cordova.js'))){
        // cordova.js is created when npm package is published
        // it is not included in git (excluded via .gitignore)
        // when platform is directly included from git or local file path, cordova.js may be missing
        events.emit('error', 'cordova.js missing');
        // cannot build cordova-js on th fly as some dependencies are missing
        // events.emit('log', 'building cordova.js at ' + rootDir);
        //execa.sync('npm', ['run', 'build'],{cwd: rootDir});
        //execa.sync('npx', ['cordova-js build', '>', 'bin/templates/platform_www/cordova.js'],{cwd: rootDir});
    }
