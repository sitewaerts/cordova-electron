    ...
    framework: {
        install: (obj, plugin_dir, project_dir, plugin_id, options) => {
            ...
            if (
                ...
                events.emit(
                    'warn',
                    `[Cordova Electron] The service name "${serviceName}" is already taken by "${appPackage.cordova.services[serviceName].pluginId}" and can not be redeclared.`
                );
                return;
            }
        ...
        uninstall: (obj, plugin_dir, project_dir, plugin_id, options) => {
            ...
            if (
                ...
                Object.keys(appPackage.cordova.services).forEach(serviceName => {
                    if (appPackage.cordova.services[serviceName].electronModule === electronPluginName) {
