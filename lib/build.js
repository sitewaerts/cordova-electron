...
class ElectronBuilder {
    ...
    configure () {
         ...
         */
        this.buildSettings = this.configureUserBuildSettings().configureBuildSettings();
    ...
    configureUserBuildSettings () {
        if (this.buildConfig && this.buildConfig.electron) {
            let userBuildSettings = {config: {}};

            if(this.isDevelopment)
            {
                // For testing builds, use 'store' to reduce build time significantly.
                userBuildSettings.config.compression = 'store';
            }

            userBuildSettings.config.removePackageScripts = true;
            ...
            for (const platform in this.buildConfig.electron) {
                 ...
                 * Any invalid property will be warned and ignored.
                 * If there are no valid properties
                 */
    ...
    __appendUserSigning (platform, signingConfigs, userBuildSettings) {
        ...
        if (platform === 'win' && config) {
            if(BuildJsonParser.support.SigntoolOptions)
            {
                userBuildSettings.config.win.signtoolOptions = {};
                this.__appendWindowsUserSigning(config, userBuildSettings.config.win.signtoolOptions);
            }
            else {
    ...
    __appendWindowsUserSigning (config, signtoolOptions) {
        const certificateFile = config.certificateFile;

        if (certificateFile && fs.existsSync(certificateFile)) {
            signtoolOptions.certificateFile = certificateFile;

        if (config.certificatePassword || process.env.CSC_KEY_PASSWORD) signtoolOptions.certificatePassword = config.certificatePassword || process.env.CSC_KEY_PASSWORD;
        } else if (certificateFile) {
            events.emit('warn', `The provided certificate file does not exist in the given path => ${certificateFile}`);
        }

        if (config.certificateSubjectName) signtoolOptions.certificateSubjectName = config.certificateSubjectName;
        if (config.certificateSha1) signtoolOptions.certificateSha1 = config.certificateSha1;
        if (config.signingHashAlgorithms) signtoolOptions.signingHashAlgorithms = config.signingHashAlgorithms;
        ...
        if (additionalCertificateFile && fs.existsSync(additionalCertificateFile)) {
            signtoolOptions.additionalCertificateFile = additionalCertificateFile;
    ...
    configureBuildSettings () {
        const config = {};
        // base config for all platforms
        deepMerge(config, require(path.resolve(__dirname, './build/base.json')));
        // global config provided via /build.json
        deepMerge(config, this.userBuildSettings || this.fetchPlatformDefaults(process.platform));
        // config provided by plugins via /platforms/electron/build.json
        deepMerge(config, require(path.resolve(this.api.locations.root, 'build.json')));
        return config;
    }
    ...
    injectProjectConfigToBuildSettings () {
        ...
        // update build settings with formatted data
    ...
    async npmInstall() {
        execa.sync('npm', ['install'], {
            cwd: this.api.locations.www
        });
    }

    async build () {
        await this.npmInstall();
        // console.debug("starting electron-builder " + require('electron-builder/package.json').version,
        //     util.inspect(this.buildSettings, {showHidden: false, depth: null, colors: true}));
        await require('electron-builder').build(this.buildSettings);
    }
}
